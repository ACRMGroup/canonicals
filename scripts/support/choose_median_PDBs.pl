...
my %medianCluster2PDBMappingHash = ();
...
###############################################################################
# sub gather_median_PDBs: Gather mapping of PDB codes to cluster numbers.
###############################################################################

sub gather_median_PDBs
{
   # &gather_median_PDBs(\%medianCluster2PDBMappingHash);

   my $medianCluster2PDBMappingHash = $_[0];
   my $line = "";
   my $clusterNumber = -1;
   my @parts = ();
   my $rest = "";
   ...
   while($line = <CLAN>)
   {
      ...
      # Line is of the form:
      #
      #     1 /home/bsm2/abhi/CANONICALS/NEW_DATASET/NUMBERED_Fv_PDB//1nma.pdb-L24-L34
      ...
      $$medianCluster2PDBMappingHash{$clusterNumber} .= "$pdbCode";

   } # End of while($line = <CLAN>).

} # End of sub-routine "gather_median_PDBs".
...
sub write_representative_structure_and_critical_residues
{
   ...
   foreach $canonicalClassName (@$canonicalClassNames)
   {
      ...
      $pdbCode = $$medianCluster2PDBMappingHash{$clusterNumber};
