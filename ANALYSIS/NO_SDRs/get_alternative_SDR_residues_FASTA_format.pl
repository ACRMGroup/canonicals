...
my $noSDRsCanonicalClass = "";
my $alternateCanonicalClass = "";
...
if($#ARGV < 6)
{
   ...
   print STDERR "\n5. Canonical class with no SDRs";
   print STDERR "\n6. Alternate canonical class (with SDRs) for loop of same length";
   print STDERR "\n7. Output file";
   print STDERR "\n\n";
   exit(0);
}
...
$noSDRsCanonicalClass = $ARGV[4];
$alternateCanonicalClass = $ARGV[5];
...
{
   # Check if the PDB belongs to the canonical class with no SDRs defined.

   if($pdbCodesCanonicalClassesHash{$pdbCode} ne $noSDRsCanonicalClass)
   {
      next;
   }
   ...
   $canonicalClass = $alternateCanonicalClass;
