...
foreach $pdbCode (keys %pdbCodesCanonicalClassesHash)
{
   ...
   print WHD ">$pdbCode:", $SDRsHash{$pdbCode}, "\n";

   @SDRPositions = split(/:/, $SDRsHash{$pdbCode});
   ...
   foreach $position (@SDRPositions)
   {
      ...
      {
         print "PDB Code: $pdbCode, Position: $position, Residue: ", $numberingHash{$position}, "\n";
      }
   }
