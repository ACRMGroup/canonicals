...
# This script takes the results of the case 3, part 1 run (i.e. all PDBs that have
# maximum sequence identity over key residues with a query PDB) and finds the sequence
# identity over the key framework residues + loop sequence, in addition to the RMS
# in the loop region.
...
while($line = <HD>)
{
   ...
   elsif($line =~ /^[1-9]/)
   {
      ...
      print "\n$loop,$loopDefinition,$queryPDBCode,$queryCanonicalClass,$bestMatchPDBCode,$bestMatchCanonicalClass,$seqid,$rms";
