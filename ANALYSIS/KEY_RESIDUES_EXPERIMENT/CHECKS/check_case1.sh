...
do
   ...
   # Check if the PDB files can be read.

   if [ ! -r $queryPDBFilename ]
   then
      echo $line
      echo "Unable to read $queryPDBFilename"
      exit 0
   fi

   if [ ! -r $targetPDBFilename ]
   then
      echo $line
      echo "Unable to read $targetPDBFilename"
      exit 0
   fi

   # Gather the query and target loop sequences.
   ...
   sequenceIdentity=`perl $HOME/CANONICALS/acaca/ANALYSIS/ANDREW_KEY_RESIDUES_EXPERIMENT/get_sequence_identity.pl $queryLoop $targetLoop`
