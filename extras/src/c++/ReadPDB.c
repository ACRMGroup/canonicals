...
PDB *dummyReadPDB(FILE *fp,int *natom){
	PDB *pdb;
   	pdb = doReadPDB(fp, natom, TRUE, 1, 1);
   	pdb = RemoveAlternates(pdb);
   printf("called ok\n");
               ...
               int  ModelNum)
{
   ...
   /** Residue counters JMH Sept 2007 **/
   /*int      new_chain;*/
   int      reschaincount = 0;
   int      respdbcount = 0;
   int      lastresidue = 0;
   ...
   *natom         = 0;
   ...
   while(fgets(buffer,159,fp))
   {
                 ...
                 &x,&y,&z,&occ,&bval) != EOF)
      {
         ...
         {
            /* Check to see if a new chain has been read in. 14.09.07 JMH */
            if (strcmp(lastchain,chain)!=0){
                strcpy(lastchain,chain);
                reschaincount = 0;
            }
            /* Check to see if a new residue has been read in. */
            /* Is so increment the counters appropriately. 14.09.07 JMH */
            if (lastresidue!=resnum || strcmp(lastinsert,insert)!=0){
                reschaincount++;
                respdbcount++;
                lastresidue = resnum;
                strcpy(lastinsert,insert);
            }
            ...
            */
               ...
               (OccRank == 0))
            {
               ...
               p->reschaincount = reschaincount;
               p->respdbcount = respdbcount;
               p->next   = NULL;
			   ...
			   strcpy(p->line, buffer); /* jmh added this one.. */
            }
            else   /* Partial occupancy                                 */
            {
               ...
               if(NPartial < MAXPARTIAL)
               {
                  ...
                  strcpy(multi[NPartial].insert,      insert);
			   	  strcpy(multi[NPartial].line, buffer); /* jmh added this one.. */
