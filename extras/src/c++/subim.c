...
/***********************************************************************/
/* Prototypes
void det_sgpe(char *tseq, long *chainClass, long *sgpe);
...
{
   int i=0;
   long chainClass,sgpe;

   for(i=0;i<numberOfSequences;i++)
   {
      det_sgpe(aaSequences[i],&chainClass,&sgpe);

      /* The codes are as follows:

         Class: 0 - Heavy chain.
         Class: 1 - Light chain Kappa chainClass.
         Class: 2 - Heavy chain Lambda chainClass.
      */

      chainType[i] = chainClass;
...
void det_sgpe(char *tseq, long *chainClass, long *sgpe)
{
   ...
   if( strstr(sTab_res_gen[sgp1/2],"Kappa") )
      *chainClass = KAPPA;
   else
   if( strstr(sTab_res_gen[sgp1/2],"Lambda") )
      *chainClass = LAMBDA;
   else
   if( strstr(sTab_res_gen[sgp1/2],"Heavy") )
      *chainClass = HEAVY;
