/*************************************************************************
   ...
   Copyright:  (c) Dr. Andrew C. R. Martin, UCL 1994-2007
...
#include "pdb2pir.h"
#include <string.h>
/* do not understand why i have to include this prototype.. */
...
{
   fprintf(stderr,"\npdb2pir V2.7 (c) 1994-2007 Dr. Andrew C.R. Martin, \
UCL\n");
   fprintf(stderr,"\nUsage: pdb2pir [-h] [-l label] [-t title] [-s] [-c] \
[-u] [-p] [-q] [-x] [-f] [-n] [infile [outfile]]\n");
...
added in lower case\n");
   fprintf(stderr,"(or upper case if the -u flag is given).\n");

   fprintf(stderr,"\nThis -n option causes the sequence to be output \
   ...
   for(i=0; i<nchain[0]; i++)
   {
      if(!DoneSEQRES[i])
      {
         ...
         */
         ...
         if(!gQuiet)
         {
            fprintf(stderr,"Warning: Chain %c from SEQRES records not found \
in ATOM records%s%s\n",
                    seqchains[i],
                    ((gLabel[0])?" Label: ":""),
                    ((gLabel[0])?gLabel:""));
